"""Бот «Apex Master»: расширенный flow-diff планировщик поверх ``orbit_lite``
(динамический ROI, ring-conquest, проактивная защита, кометы, KNN-источники, near/far).

Перенесён дословно из ноутбука
``apex-hybrid-dynamic-ring-control-border-defense.ipynb`` (ячейка ``main.py``); изменены
только заголовок (``sys.path`` вынесен в ``agents.base``) и точка входа — добавлен класс
:class:`ApexMasterAgent` под интерфейс ``eval``. Модульный ``agent(obs)`` оставлен.
"""
...
from agents.base import ScriptedAgent  # настраивает sys.path для orbit_lite
...
# ---------------------------------------------------------------------------
# Обёртка под интерфейс eval (callable(obs, config) -> ходы)
# ---------------------------------------------------------------------------

class ApexMasterAgent(ScriptedAgent):
    """«Apex Master» под интерфейс eval (см. :class:`agents.base.ScriptedAgent`)."""

    name = "apex_master"

    def __init__(self, name: str | None = None, *, seed: int = 0) -> None:
        super().__init__(name or "apex_master", seed=seed)
        self._runtime = ProducerLiteRuntime()   # собственный рантайм (свой forecast-кэш)
    ...
    def __call__(self, obs, config=None):
        ...
        obs_tensors = single_obs_to_tensor(obs, player_id=player_id)
        with torch.no_grad():
            sparse_row = self._runtime.tensor_action(obs_tensors)
        return sparse_action_row_to_moves(sparse_row, obs, player_id=player_id)
