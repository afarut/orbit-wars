"""Бот «Producer Hybrid v4»: консервативный flow-diff планировщик поверх ``orbit_lite``.

Перенесён дословно из ноутбука ``orbit-wars-i-m-stronger.ipynb`` (ячейка ``main.py``);
изменены только заголовок (настройка ``sys.path`` вынесена в ``agents.base``) и точка
входа — добавлен класс :class:`ProducerHybridAgent` под интерфейс ``eval``
(``callable(obs, config) -> ходы``). Модульный ``agent(obs)`` оставлен как был.
"""
...
class ProducerHybridAgent(ScriptedAgent):
    """«Producer Hybrid v4» под интерфейс eval (см. :class:`agents.base.ScriptedAgent`)."""

    name = "producer_hybrid"

    def __init__(self, name: str | None = None, *, seed: int = 0) -> None:
        super().__init__(name or "producer_hybrid", seed=seed)
