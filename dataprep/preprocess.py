...
на декоде через :func:`intercept.intercept_angle`. Поэтому здесь переводим угол в
планету-назначение готовой обратной операцией :func:`core.utils.planet_at_angle`
и пишем компактный таргет ``sends = [[from_id, dest_id], ...]`` (угол выкидываем).
...
--------------------------------------
    {
      ...
      "sends":      [[from_id, dest_id]],   # резолвнутые вылеты (hold -> [])
...
from core.utils import planet_at_angle
...
def resolve_sample(sample: dict) -> dict:
    ...
    счётчики для агрегированной статистики прогона.
    """
    state = sample["state"]
    planets = state["planets"]
    av = float(state.get("angular_velocity", 0.0) or 0.0)
    comets = state.get("comets") or []
    player = int(state.get("player", 0) or 0)

    id2idx = {int(p[0]): i for i, p in enumerate(planets)}

    sends = {}                # from_id -> dest_id (dict гасит дубль источника)
    unresolved = []
    for move in sample.get("action") or []:
        from_id, angle, ships = int(move[0]), float(move[1]), int(move[2])
        from_idx = id2idx.get(from_id)
        if from_idx is None:
            unresolved.append(from_id)   # источника нет в obs — аномалия, не учим
            continue
        dest_idx = planet_at_angle(
            planets, from_idx, angle, ships, angular_velocity=av, comets=comets)
        if dest_idx is None:
            unresolved.append(from_id)   # солнце/выход за поле/не восстановилось
            continue
        sends[from_id] = int(planets[dest_idx][0])
    ...
    out = {
        "state": state,
        "sends": [[fid, did] for fid, did in sends.items()],
...
def run(in_path: str, out_path: str, limit: Optional[int] = None,
        insights_path: Optional[str] = None) -> Dict[str, int]:
    ...
    with open(in_path, "r", encoding="utf-8") as fin, \
        ...
        for i, line in enumerate(tqdm(fin, desc="resolve angle->planet")):
            ...
            if not line:
                continue
            rec = resolve_sample(json.loads(line))
            st = rec.pop("_stats")
            agg["states"] += 1
